...
var generateCreep = {
    run: function (){
        ...
        StructureSpawn.prototype.createMiner = function(sourceId,name,role,partsOption) {
            // long distance harvesters
            return this.spawnCreep([WORK,WORK,WORK,WORK,WORK,MOVE], name, {memory: {role: role, sourceId: sourceId, parts: partsOption}});
        }
        StructureSpawn.prototype.createLorry = function(totalEnergy,name,role,partsOption) {
            // long distance harvesters
            let body = [];
            let numParts = Math.floor(totalEnergy / 150);
